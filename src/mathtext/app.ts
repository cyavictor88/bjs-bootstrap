...
class App {
    ...
    constructor() {
       ...
       // var [mathml, mmp] = trykatex("\\begin{bmatrix} a & b_{xyz} & c \\\\  d^{qp} & e & foo \\end{bmatrix}");
         var [mathml, mmp] = trykatex("\\text{a b}=\\int_{a}^{b} f(x) \\, dx = \\begin{cases} \\textrm{true,} & \\textrm{if } 0 < x_u < 35^{kmm} \\\\ \\textrm{false,} & \\textrm{otherwise} x^y \\end{cases} = \\oint_V f(s) = \\int_{a}^{b} f(x) \\, dx  =\\sum_{n=1}^{\\infty}n=\\lim_{x \\to \\infty} sin(x) = \\begin{bmatrix} annnn^y & b_{44}  \\\\ 5^5_3 & zzzz  \\end{bmatrix}");
