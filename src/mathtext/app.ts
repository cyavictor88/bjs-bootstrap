...
class App {
    ...
    constructor() {
        ...
        var [mathml,mmp]=trykatex("\\begin{bmatrix} 1_{xy} & 2_x & 3 \\\\ a & b & c \\end{bmatrix}= \\begin{bmatrix} n & u & 6 \\\\ a & \\begin{bmatrix} q & k \\\\ p & e \\end{bmatrix} & c \\\\ z & w & q  \\\\ 8 & 8 & 8 \\end{bmatrix}=\\vec{a_z}=\\lim_{x\\to\\infty} sin(\\theta)= \\vert_{-5}^{x17}= \\vec{a_z} =\\sum_{n=1}^{\\infty} 2^{-n} = 1_{x}^5=2 x + 88y =c_{iz \\pi} ");
