...
class App {
    ...
    constructor() {
        ...
        var [mathml, mmp] = trykatex("\\text{a b x}=\\int_{a}^{b} f(x) \\, dx = \\begin{cases} \\textrm{true,} & \\textrm{if } 0 < x < 5 \\\\ \\textrm{false,} & \\textrm{otherwise} \\end{cases} = \\oint_V f(s) = \\int_{a}^{b} f(x) \\, dx  =\\sum_{n=1}^{\\infty}=\\lim_{x \\to \\infty} sin(x) = \\begin{bmatrix} annnn^y & b_{44}  \\\\ 5^5_3 & zzzzz  \\end{bmatrix}");
